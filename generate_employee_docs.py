#!/usr/bin/env python3
"""
Employee Endpoints Documentation Generator
Analyzes the codebase and generates comprehensive API documentation
"""
import os
import sys
import json
...
class EmployeeEndpointsDocumenter:
    def __init__(self):
        self.endpoints = []
        self.analyze_endpoints()
    
    def analyze_endpoints(self):
        """Analyze all employee-related endpoints from the codebase"""
        ...
        # Employee Bookings (Own + Assigned)
        self.endpoints.append({
            'method': 'GET',
            'endpoint': '/api/booking/bookings/',
        ...
        self.endpoints.append({
            'method': 'GET',
            'endpoint': '/api/booking/bookings/{id}/',
        ...
        self.endpoints.append({
            'method': 'POST',
            'endpoint': '/api/booking/bookings/{id}/status/',
        ...
        # ============ ADMIN ENDPOINTS (for managing employees) ============
        
        # List All Employees (Admin)
        self.endpoints.append({
            'method': 'GET',
            'endpoint': '/api/booking/admin/employees/',
        ...
        # Assign Booking to Employee (Admin)
        self.endpoints.append({
            'method': 'POST',
            'endpoint': '/api/booking/admin/bookings/{booking_id}/assign/',
        ...
        # Admin Booking Dashboard with Employee Metrics
        self.endpoints.append({
            'method': 'GET',
            'endpoint': '/api/booking/admin/dashboard/',
        ...
        # Admin Booking Management (List All)
        self.endpoints.append({
            'method': 'GET',
            'endpoint': '/api/booking/admin/bookings/',
    ...
    def generate_documentation(self):
        ...
        doc_content = """# Employee Endpoints API Documentation
...
"""
        ...
        auth_endpoints = [ep for ep in self.endpoints if ep['endpoint'].startswith('/api/auth/')]
        employee_endpoints = [ep for ep in self.endpoints if ep['endpoint'].startswith('/api/booking/bookings')]
        admin_endpoints = [ep for ep in self.endpoints if 'admin' in ep['endpoint']]
        
        # Employee Authentication & Profile Management
        doc_content += "## Employee Authentication & Profile Management\n\n"
        for endpoint in auth_endpoints:
            doc_content += self.format_endpoint(endpoint)
        
        # Employee Booking Management
        doc_content += "## Employee Booking Management\n\n"
        for endpoint in employee_endpoints:
            doc_content += self.format_endpoint(endpoint)
        
        # Admin Employee Management
        doc_content += "## Admin Employee Management\n\n"
...
"""
    ...
    def format_endpoint(self, endpoint):
        ...
        content += f"**Authentication**: {'Required' if endpoint['auth_required'] else 'Not Required'}\n"
        content += f"**Admin Only**: {'Yes' if endpoint['admin_only'] else 'No'}\n\n"
        
        if 'query_params' in endpoint:
            content += "**Query Parameters**:\n"
            for param, desc in endpoint['query_params'].items():
                content += f"- `{param}`: {desc}\n"
            content += "\n"
        
        if endpoint.get('payload'):
            content += "**Request Payload**:\n```json\n"
            content += json.dumps(endpoint['payload'], indent=2)
            content += "\n```\n\n"
        
        if endpoint.get('response_example'):
            content += f"**Response** (Status: {endpoint['status_code']}):\n```json\n"
            content += json.dumps(endpoint['response_example'], indent=2)
            content += "\n```\n\n"
    ...
    def run(self):
        ...
        print("Generating Employee Endpoints Documentation...")
        
        doc_content = self.generate_documentation()
        
        # Write to file
        filename = "EMPLOYEE_ENDPOINTS_COMPREHENSIVE_DOCS.md"
        ...
        # Print summary
        auth_count = len([ep for ep in self.endpoints if ep['endpoint'].startswith('/api/auth/')])
        booking_count = len([ep for ep in self.endpoints if 'booking' in ep['endpoint'] and 'admin' not in ep['endpoint']])
        admin_count = len([ep for ep in self.endpoints if 'admin' in ep['endpoint']])
        
        print(f"\n📊 Endpoint Categories:")
        print(f"   • Authentication & Profile: {auth_count}")
        print(f"   • Employee Booking Management: {booking_count}")
        print(f"   • Admin Employee Management: {admin_count}")
        
        return filename

if __name__ == "__main__":
    documenter = EmployeeEndpointsDocumenter()
    documenter.run()